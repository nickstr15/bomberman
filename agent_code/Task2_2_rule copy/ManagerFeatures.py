...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    single_inverse_coin_distance = inv_coins[hot_one]
    inv_coins[inv_coins>=0] = 0
    inv_coins[hot_one] = number_of_coins + max_len_coins + 1/3 * (number_of_crates + max_len_crates)

    features = np.append(features, inv_coins)

    # append the inverse distance to the goal coin to be able to compare to the crates
    features = np.append(features, single_inverse_coin_distance)

    # append the crates features
    goodness = inv_crate_distances * crate_points
    reduced_goodness = np.max(goodness, axis=1)
    hot_one = np.argmax(reduced_goodness)
    single_goodness = reduced_goodness[hot_one]
    reduced_goodness[reduced_goodness >= 0] = 0
    reduced_goodness[hot_one] = number_of_coins + max_len_coins + 1/3 * (number_of_crates + max_len_crates)

    features = np.append(features, reduced_goodness)

    # append the inverse distance to the goal crate to be able to compare to the coins
    features = np.append(features, single_goodness)
    ...
    if future_explosion_map[player_pos[0], player_pos[1]] == 1:
        ...
        if pos[0] <= player_pos[0]:   # -> pos[1] == player_pos[1]
            ...
            if field[player_pos[0], player_pos[1]+1] == 0:
                running[2] = 3        # -> run around a corner -> Safe
            if field[player_pos[0], player_pos[1]-1] == 0:
                running[3] = 3        # -> run around a corner -> Safe
        ...
        if pos[1] <= player_pos[1]:   # -> pos[0] == player_pos[0]
            ...
            if field[player_pos[0]+1, player_pos[1]] == 0:
                running[0] = 3        # -> run around a corner -> Safe
            if field[player_pos[0]-1, player_pos[1]] == 0:
                running[1] = 3        # -> run around a corner -> Safe
    ...
    # append a feature, which indicates if we have our bomb  (needs to be invese since all distances are passed inverse)
    if not game_state["self"][2]:
        # print(self.bomb_timer)
        # assert self.bomb_timer != 0
        if self.bomb_timer == 0:
            features = np.append(features, 1)
            self.logger.info("Bombtimer haette eine Division durch 0 ausgeloest")
        else:
            features = np.append(features, 1/self.bomb_timer)
    else:
        # print(self.bomb_timer)
        # assert self.bomb_timer == 0
