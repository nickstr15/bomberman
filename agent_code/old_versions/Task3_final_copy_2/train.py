...
LOAD = 'save after 14000 iterations'
...
def setup_training(self):
    ...
    self.episode_counter = 14000
    self.total_episodes = TRAINING_EPISODES

    self.pos_saver = [] #store the last 10 positions

    self.game_score = 0 
    self.game_score_arr = np.loadtxt("game_score_14000").tolist()
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    # play 20 games to fill the experience buffer
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if self.episode_counter % (TRAINING_EPISODES // 100) == 0: #save parameters 2 times
