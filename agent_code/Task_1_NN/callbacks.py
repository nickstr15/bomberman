...
import numpy as np
from .NNModel import DeepQNetwork, state_to_features
...
def setup(self):
    ...
    self.network = DeepQNetwork(LEARNING_RATE, DISCOUNTING_FACTOR, 
                            ...
                            N, LOSS_FUNCTION, OPTIMIZER)
                        
    self.epsilon = np.linspace(self.network.epsilon_begin, self.network.epsilon_end, 100)
    self.counter = 0
